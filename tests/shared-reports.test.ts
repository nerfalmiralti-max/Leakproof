...
const original = new Blob([readFileSync("tests/fixtures/observation.webm")], {
  type: "video/webm",
});
const input = {
  analysis: scoreEvidence({
    quality: "GOOD",
    qualityIssues: [],
    waterDetected: true,
  ...
  locationLabel: "Water near path",
  description: "Observed water",
  video: {
    name: "original.webm",
    type: "video/webm",
    size: original.size,
...
const request = (body?: unknown, authorized = false) =>
  new Request("http://localhost/api/reports", {
    method: body === undefined ? "GET" : "POST",
    headers: {
      ...(body === undefined ? {} : { "Content-Type": "application/json" }),
      ...(authorized ? { Authorization: "Bearer dispatcher-test-key" } : {}),
    },
...
beforeEach(() => {
  ...
  storage.createSignedUploadUrl.mockResolvedValue({
    data: { signedUrl: "https://storage.example/upload?token=upload-only" },
    error: null,
  });
  ...
  storage.createSignedUrl.mockResolvedValue({
    data: { signedUrl: "https://storage.example/video?token=private" },
    error: null,
  });
...
describe("private shared reports", () => {
  it("uploads the exact original bytes and confirms only after remote insertion", async () => {
    ...
    expect(await remoteRepository.create(input, original)).toEqual(report);
    expect(fetchMock).toHaveBeenCalledTimes(3);
    expect(fetchMock.mock.calls[2][0]).toBe("/api/reports");
  });
  it("missing Supabase configuration returns a safe service error", async () => {
    const actual = await vi.importActual<
      typeof import("@/lib/reports/server-client")
    >("@/lib/reports/server-client");
    vi.stubEnv("NEXT_PUBLIC_SUPABASE_URL", "");
    vi.stubEnv("SUPABASE_SERVICE_ROLE_KEY", "");
    vi.mocked(getSupabase).mockImplementationOnce(actual.getSupabase);
    expect((await prepare(request(input))).status).toBe(503);
    expect(storage.createSignedUploadUrl).not.toHaveBeenCalled();
  });
  it("wrong dispatcher key cannot access reports", async () => {
    expect(
      (
        await list(
          new Request("http://localhost/api/dispatcher/reports", {
  ...
  it("missing evidence report returns 404 without signing", async () => {
    expect(
      (
        await evidence(request(undefined, true), {
  ...
  it("network failure is explicit and does not retry or submit locally", async () => {
    const fetchMock = vi.fn().mockRejectedValue(new Error("network down"));
    vi.stubGlobal("fetch", fetchMock);
    await expect(remoteRepository.create(input, original)).rejects.toThrow(
      "Network error",
    );
  ...
  it("validates and saves a real report, without returning its storage path", async () => {
    ...
    expect(storage.createSignedUploadUrl).toHaveBeenCalledWith(
      `reports/${report.id}/evidence.webm`,
      { upsert: false },
    );
    expect(insert).toHaveBeenCalledTimes(1);
  });
  it.each(["NO_WATER", "UNCERTAIN"])(
    "rejects %s before creating an upload",
    async (risk) => {
      expect(
        (
          await prepare(
            request({
              ...input,
              analysis: { ...input.analysis, leakRisk: risk },
            }),
          )
        ).status,
      ).toBe(400);
  ...
  it.each([
    [
      "unsupported MIME",
      { ...input, video: { ...input.video, type: "text/plain" } },
    ],
    [
      "oversized video",
      { ...input, video: { ...input.video, size: 52428801 } },
    ],
    ["invalid location", { ...input, location: { lat: 100, lng: 51 } }],
    [
      "fabricated HIGH risk",
      { ...input, analysis: { ...input.analysis, leakRisk: "HIGH" } },
    ],
    ["demo", { ...input, analysis: createDemoResult("HIGH") }],
    [
      "client storage path",
      { ...input, evidence_path: "another-report/video.webm" },
    ],
  ...
  it("failed or missing upload does not insert a report", async () => {
    ...
    expect((await submit(request({ ticket: await ticket() }))).status).toBe(
      400,
    );
    expect(insert).not.toHaveBeenCalled();
  });
  ...
  it("rejects forged file contents even when the MIME and filename claim video", async () => {
    storage.download.mockResolvedValue({
      data: new Blob([new Uint8Array(original.size)], { type: "video/webm" }),
      error: null,
    });
    expect((await submit(request({ ticket: await ticket() }))).status).toBe(
      415,
  ...
  it("rejects every dispatcher operation without a key", async () => {
    ...
    expect(
      (await status(request({ status: "IN_REVIEW" }), context)).status,
    ).toBe(401);
    expect(getSupabase).not.toHaveBeenCalled();
    expect(storage.createSignedUrl).not.toHaveBeenCalled();
  });
  it("correct dispatcher key lists reports and persists status", async () => {
    const report = await created();
    expect(
      (await (await list(request(undefined, true))).json()).reports,
    ).toHaveLength(1);
    const response = await status(request({ status: "IN_REVIEW" }, true), {
      params: Promise.resolve({ id: report.id }),
    });
    expect(response.status).toBe(200);
    expect(
      (await (await list(request(undefined, true))).json()).reports[0].status,
    ).toBe("IN_REVIEW");
  });
  it("rejects invalid status values", async () => {
    ...
    expect(
      (
        await status(request({ status: "DELETED" }, true), {
          params: Promise.resolve({ id: report.id }),
        })
      ).status,
    ).toBe(400);
  ...
  it("creates a five-minute evidence URL only after dispatcher authorization", async () => {
    const report = await created();
    const response = await evidence(request(undefined, true), {
      params: Promise.resolve({ id: report.id }),
    });
    expect(response.status).toBe(200);
  ...
  it("remote repository rejects demo evidence before making network calls", async () => {
    ...
    await expect(
      remoteRepository.create(
        { ...input, analysis: createDemoResult("HIGH") },
        original,
      ),
    ).rejects.toThrow();
